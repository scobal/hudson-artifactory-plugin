...
public abstract class ActionableHelper {
    ...
    /**
     * Get a list of {@link hudson.tasks.Builder}s that are related to the project.
     *
     ...
     */
    public static <T extends Builder> List<T> getBuilders(IBaseBuildableProject project, Class<T> type) {
    ...
    public static String getUserCausePrincipal(AbstractBuild build, String defaultPrincipal) {
        Cause.UserCause userCause = getUserCause(build);
        String principal = defaultPrincipal;
        if (userCause != null && userCause.getUserName() != null) {
            principal = userCause.getUserName();
        }
        return principal;
    }

    private static Cause.UserCause getUserCause(AbstractBuild build) {
        CauseAction action = ActionableHelper.getLatestAction(build, CauseAction.class);
        if (action != null) {
            for (Cause cause : action.getCauses()) {
